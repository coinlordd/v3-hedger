...
(function generateABI() {
  const filePaths = loopDirectory("./artifacts/contracts");

  // filePaths can contain anything -> pick .json files and ignore .dbg.json files
  let paths = filePaths.filter(file => file.endsWith(".json") && !file.endsWith(".dbg.json"));
  ...
  fs.writeFileSync("./src/data/abi.json", finalAbi);
  console.log("ABI written to src/data/abi.json");
})();
